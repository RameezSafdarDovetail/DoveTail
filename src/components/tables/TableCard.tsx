...
export function TableCard({ columnsClassName, headers, children }: TableCardProps) {
  return (
    <div className={cn(ui.glass, 'overflow-hidden rounded-default max-[640px]:overflow-x-auto')}>
      <div
        className={cn(
      ...
      >
        {headers.map((header) => (
          <span key={header} className="text-[11px] font-bold tracking-[0.06em] text-text-3 uppercase">
            {header}
          </span>
        ))}
      </div>
...
export function TableRow({ columnsClassName, muted = false, children, onClick }: TableRowProps) {
  return (
    <div
      className={cn(
